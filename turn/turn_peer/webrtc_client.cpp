...
public:
  ...
  Impl(const std::string &url, const CameraConfig &config,
       const WebRTCClient::IceConfig &ice)
      : server_url(url), camera_config(config), ice_config(ice) {
  ...
  static void on_negotiation_needed(GstElement *webrtcbin, gpointer user_data) {
    ...
    std::cout << "[client] on-negotiation-needed\n";
...
WebRTCClient::WebRTCClient(const std::string &server_url)
    : pImpl(std::make_unique<Impl>(server_url, CameraConfig{}, IceConfig{})) {}

WebRTCClient::WebRTCClient(const std::string &server_url,
                           const CameraConfig &camera_config)
    : pImpl(
          std::make_unique<Impl>(server_url, camera_config, IceConfig{})) {}

WebRTCClient::WebRTCClient(const std::string &server_url,
                           const CameraConfig &camera_config,
                           const IceConfig &ice_config)
    : pImpl(std::make_unique<Impl>(server_url, camera_config, ice_config)) {}
...
bool WebRTCClient::start() {
  ...
  if (pImpl->ice_config.force_relay &&
      pImpl->ice_config.turn_server.empty()) {
    pImpl->reportError("TURN server required when relay-only is enabled");
    return false;
  }
  ...
  g_object_set(pImpl->webrtcbin, "bundle-policy",
               GST_WEBRTC_BUNDLE_POLICY_MAX_BUNDLE, nullptr);
  if (pImpl->ice_config.force_relay) {
    g_object_set(pImpl->webrtcbin, "ice-transport-policy",
                 GST_WEBRTC_ICE_TRANSPORT_POLICY_RELAY, nullptr);
  }
  if (!pImpl->ice_config.stun_server.empty()) {
    g_object_set(pImpl->webrtcbin, "stun-server",
                 pImpl->ice_config.stun_server.c_str(), nullptr);
  }
  if (!pImpl->ice_config.turn_server.empty()) {
    g_object_set(pImpl->webrtcbin, "turn-server",
                 pImpl->ice_config.turn_server.c_str(), nullptr);
  }
...
void WebRTCClient::setIceConfig(const IceConfig &config) {
  if (pImpl->pipeline || pImpl->webrtcbin) {
    pImpl->emitError("Cannot change ICE config while running");
    return;
  }
  pImpl->ice_config = config;
}
...
void WebRTCClient::Impl::sendIceCandidate(guint mlineindex,
  ...
  if (std::strstr(candidate, " TCP ") != nullptr ||
    ...
    std::cout << "[client] TCP ICE candidate detected (sending)\n";
  }
...
void WebRTCClient::Impl::handleOfferCreated(GstPromise *promise) {
  ...
  std::cout << "[client] Sending offer to signaling server\n";
  if (!transport) {
  ...
  if (!transport->sendOffer(sdp_str, webrtc_id, &answer_sdp, &error_msg)) {
    reportError(error_msg.empty() ? "Failed to get answer from server"
                                  : error_msg);
    std::cout << "[client] Offer send failed: "
              << (error_msg.empty() ? "unknown" : error_msg) << "\n";
    g_free(sdp_str);
    gst_webrtc_session_description_free(offer);
    return;
  }
  std::cout << "[client] Offer send ok, answer received\n";
...
void WebRTCClient::Impl::handleIceCandidate(GstElement *webrtcbin,
  ...
  if (candidate && *candidate) {
    std::cout << "[client] Local ICE candidate generated\n";
    sendIceCandidate(mlineindex, candidate);
  } else {
    std::cout << "[client] ICE candidate empty\n";
  }
}
...
void WebRTCClient::Impl::handleWebRTCStateChanged(GObject *obj) {
  ...
  g_object_get(obj, "ice-gathering-state", &ice_gather_state, nullptr);
  g_object_get(obj, "ice-connection-state", &ice_conn_state, nullptr);

  const char *gather_str = "unknown";
  switch (ice_gather_state) {
  case GST_WEBRTC_ICE_GATHERING_STATE_NEW:
    gather_str = "new";
    break;
  case GST_WEBRTC_ICE_GATHERING_STATE_GATHERING:
    gather_str = "gathering";
    break;
  case GST_WEBRTC_ICE_GATHERING_STATE_COMPLETE:
    gather_str = "complete";
    break;
  }
  std::cout << "[client] ICE gathering state: " << gather_str << "\n";
